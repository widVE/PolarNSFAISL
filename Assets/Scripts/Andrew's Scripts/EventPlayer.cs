	    ...
	    if(IsEventPlaying())
        {
			...
			if (eventsPlaying[currEventNumber].eventIndex < events[currEventNumber].eventData.Count && eventsPlaying[currEventNumber].advancedIndex)
			{
				...
				if (d != null)
				{
					...
					if (dc != null)
					{
						float charge = Mathf.Log (200.0f * events [currEventNumber].eventData [eventsPlaying [currEventNumber].eventIndex].charge * events [currEventNumber].eventData [eventsPlaying [currEventNumber].eventIndex].charge);
